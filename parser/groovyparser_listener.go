...
type GroovyParserListener interface {
	...
	// EnterEmptyDims is called when entering the emptyDims production.
	EnterEmptyDims(c *EmptyDimsContext)

	// EnterEmptyDimsOpt is called when entering the emptyDimsOpt production.
	EnterEmptyDimsOpt(c *EmptyDimsOptContext)
	...
	// EnterYieldStmtAlt is called when entering the yieldStmtAlt production.
	EnterYieldStmtAlt(c *YieldStmtAltContext)
	...
	// EnterSwitchBlockStatementExpressionGroup is called when entering the switchBlockStatementExpressionGroup production.
	EnterSwitchBlockStatementExpressionGroup(c *SwitchBlockStatementExpressionGroupContext)

	// EnterSwitchExpressionLabel is called when entering the switchExpressionLabel production.
	EnterSwitchExpressionLabel(c *SwitchExpressionLabelContext)
	...
	// EnterImplicationExprAlt is called when entering the implicationExprAlt production.
	EnterImplicationExprAlt(c *ImplicationExprAltContext)

	// EnterCastExprAlt is called when entering the castExprAlt production.
	EnterCastExprAlt(c *CastExprAltContext)

	// EnterSwitchExprAlt is called when entering the switchExprAlt production.
	EnterSwitchExprAlt(c *SwitchExprAltContext)
	...
	// EnterCastExprAltOperand is called when entering the castExprAltOperand production.
	EnterCastExprAltOperand(c *CastExprAltOperandContext)

	// EnterPostfixExprAltOperand is called when entering the postfixExprAltOperand production.
	EnterPostfixExprAltOperand(c *PostfixExprAltOperandContext)

	// EnterUnaryNotExprAltOperand is called when entering the unaryNotExprAltOperand production.
	EnterUnaryNotExprAltOperand(c *UnaryNotExprAltOperandContext)

	// EnterUnaryAddExprAltOperand is called when entering the unaryAddExprAltOperand production.
	EnterUnaryAddExprAltOperand(c *UnaryAddExprAltOperandContext)
	...
	// EnterIdentifierPrmrAltNamedPropertyArgPrimary is called when entering the identifierPrmrAltNamedPropertyArgPrimary production.
	EnterIdentifierPrmrAltNamedPropertyArgPrimary(c *IdentifierPrmrAltNamedPropertyArgPrimaryContext)

	// EnterLiteralPrmrAltNamedPropertyArgPrimary is called when entering the literalPrmrAltNamedPropertyArgPrimary production.
	EnterLiteralPrmrAltNamedPropertyArgPrimary(c *LiteralPrmrAltNamedPropertyArgPrimaryContext)

	// EnterGstringPrmrAltNamedPropertyArgPrimary is called when entering the gstringPrmrAltNamedPropertyArgPrimary production.
	EnterGstringPrmrAltNamedPropertyArgPrimary(c *GstringPrmrAltNamedPropertyArgPrimaryContext)

	// EnterParenPrmrAltNamedPropertyArgPrimary is called when entering the parenPrmrAltNamedPropertyArgPrimary production.
	EnterParenPrmrAltNamedPropertyArgPrimary(c *ParenPrmrAltNamedPropertyArgPrimaryContext)

	// EnterListPrmrAltNamedPropertyArgPrimary is called when entering the listPrmrAltNamedPropertyArgPrimary production.
	EnterListPrmrAltNamedPropertyArgPrimary(c *ListPrmrAltNamedPropertyArgPrimaryContext)

	// EnterMapPrmrAltNamedPropertyArgPrimary is called when entering the mapPrmrAltNamedPropertyArgPrimary production.
	EnterMapPrmrAltNamedPropertyArgPrimary(c *MapPrmrAltNamedPropertyArgPrimaryContext)

	// EnterIdentifierPrmrAltNamedArgPrimary is called when entering the identifierPrmrAltNamedArgPrimary production.
	EnterIdentifierPrmrAltNamedArgPrimary(c *IdentifierPrmrAltNamedArgPrimaryContext)

	// EnterLiteralPrmrAltNamedArgPrimary is called when entering the literalPrmrAltNamedArgPrimary production.
	EnterLiteralPrmrAltNamedArgPrimary(c *LiteralPrmrAltNamedArgPrimaryContext)

	// EnterGstringPrmrAltNamedArgPrimary is called when entering the gstringPrmrAltNamedArgPrimary production.
	EnterGstringPrmrAltNamedArgPrimary(c *GstringPrmrAltNamedArgPrimaryContext)

	// EnterIdentifierPrmrAltCommandPrimary is called when entering the identifierPrmrAltCommandPrimary production.
	EnterIdentifierPrmrAltCommandPrimary(c *IdentifierPrmrAltCommandPrimaryContext)

	// EnterLiteralPrmrAltCommandPrimary is called when entering the literalPrmrAltCommandPrimary production.
	EnterLiteralPrmrAltCommandPrimary(c *LiteralPrmrAltCommandPrimaryContext)

	// EnterGstringPrmrAltCommandPrimary is called when entering the gstringPrmrAltCommandPrimary production.
	EnterGstringPrmrAltCommandPrimary(c *GstringPrmrAltCommandPrimaryContext)
	...
	// EnterNamedPropertyArgList is called when entering the namedPropertyArgList production.
	EnterNamedPropertyArgList(c *NamedPropertyArgListContext)

	// EnterMapEntry is called when entering the mapEntry production.
	EnterMapEntry(c *MapEntryContext)

	// EnterNamedPropertyArg is called when entering the namedPropertyArg production.
	EnterNamedPropertyArg(c *NamedPropertyArgContext)
	...
	// EnterNamedPropertyArgLabel is called when entering the namedPropertyArgLabel production.
	EnterNamedPropertyArgLabel(c *NamedPropertyArgLabelContext)

	// EnterNamedArgLabel is called when entering the namedArgLabel production.
	EnterNamedArgLabel(c *NamedArgLabelContext)
	...
	// EnterEnhancedArgumentListInPar is called when entering the enhancedArgumentListInPar production.
	EnterEnhancedArgumentListInPar(c *EnhancedArgumentListInParContext)

	// EnterFirstArgumentListElement is called when entering the firstArgumentListElement production.
	EnterFirstArgumentListElement(c *FirstArgumentListElementContext)
	...
	// ExitEmptyDims is called when exiting the emptyDims production.
	ExitEmptyDims(c *EmptyDimsContext)

	// ExitEmptyDimsOpt is called when exiting the emptyDimsOpt production.
	ExitEmptyDimsOpt(c *EmptyDimsOptContext)
	...
	// ExitYieldStmtAlt is called when exiting the yieldStmtAlt production.
	ExitYieldStmtAlt(c *YieldStmtAltContext)
	...
	// ExitSwitchBlockStatementExpressionGroup is called when exiting the switchBlockStatementExpressionGroup production.
	ExitSwitchBlockStatementExpressionGroup(c *SwitchBlockStatementExpressionGroupContext)

	// ExitSwitchExpressionLabel is called when exiting the switchExpressionLabel production.
	ExitSwitchExpressionLabel(c *SwitchExpressionLabelContext)
	...
	// ExitImplicationExprAlt is called when exiting the implicationExprAlt production.
	ExitImplicationExprAlt(c *ImplicationExprAltContext)

	// ExitCastExprAlt is called when exiting the castExprAlt production.
	ExitCastExprAlt(c *CastExprAltContext)

	// ExitSwitchExprAlt is called when exiting the switchExprAlt production.
	ExitSwitchExprAlt(c *SwitchExprAltContext)
	...
	// ExitCastExprAltOperand is called when exiting the castExprAltOperand production.
	ExitCastExprAltOperand(c *CastExprAltOperandContext)

	// ExitPostfixExprAltOperand is called when exiting the postfixExprAltOperand production.
	ExitPostfixExprAltOperand(c *PostfixExprAltOperandContext)

	// ExitUnaryNotExprAltOperand is called when exiting the unaryNotExprAltOperand production.
	ExitUnaryNotExprAltOperand(c *UnaryNotExprAltOperandContext)

	// ExitUnaryAddExprAltOperand is called when exiting the unaryAddExprAltOperand production.
	ExitUnaryAddExprAltOperand(c *UnaryAddExprAltOperandContext)
	...
	// ExitIdentifierPrmrAltNamedPropertyArgPrimary is called when exiting the identifierPrmrAltNamedPropertyArgPrimary production.
	ExitIdentifierPrmrAltNamedPropertyArgPrimary(c *IdentifierPrmrAltNamedPropertyArgPrimaryContext)

	// ExitLiteralPrmrAltNamedPropertyArgPrimary is called when exiting the literalPrmrAltNamedPropertyArgPrimary production.
	ExitLiteralPrmrAltNamedPropertyArgPrimary(c *LiteralPrmrAltNamedPropertyArgPrimaryContext)

	// ExitGstringPrmrAltNamedPropertyArgPrimary is called when exiting the gstringPrmrAltNamedPropertyArgPrimary production.
	ExitGstringPrmrAltNamedPropertyArgPrimary(c *GstringPrmrAltNamedPropertyArgPrimaryContext)

	// ExitParenPrmrAltNamedPropertyArgPrimary is called when exiting the parenPrmrAltNamedPropertyArgPrimary production.
	ExitParenPrmrAltNamedPropertyArgPrimary(c *ParenPrmrAltNamedPropertyArgPrimaryContext)

	// ExitListPrmrAltNamedPropertyArgPrimary is called when exiting the listPrmrAltNamedPropertyArgPrimary production.
	ExitListPrmrAltNamedPropertyArgPrimary(c *ListPrmrAltNamedPropertyArgPrimaryContext)

	// ExitMapPrmrAltNamedPropertyArgPrimary is called when exiting the mapPrmrAltNamedPropertyArgPrimary production.
	ExitMapPrmrAltNamedPropertyArgPrimary(c *MapPrmrAltNamedPropertyArgPrimaryContext)

	// ExitIdentifierPrmrAltNamedArgPrimary is called when exiting the identifierPrmrAltNamedArgPrimary production.
	ExitIdentifierPrmrAltNamedArgPrimary(c *IdentifierPrmrAltNamedArgPrimaryContext)

	// ExitLiteralPrmrAltNamedArgPrimary is called when exiting the literalPrmrAltNamedArgPrimary production.
	ExitLiteralPrmrAltNamedArgPrimary(c *LiteralPrmrAltNamedArgPrimaryContext)

	// ExitGstringPrmrAltNamedArgPrimary is called when exiting the gstringPrmrAltNamedArgPrimary production.
	ExitGstringPrmrAltNamedArgPrimary(c *GstringPrmrAltNamedArgPrimaryContext)

	// ExitIdentifierPrmrAltCommandPrimary is called when exiting the identifierPrmrAltCommandPrimary production.
	ExitIdentifierPrmrAltCommandPrimary(c *IdentifierPrmrAltCommandPrimaryContext)

	// ExitLiteralPrmrAltCommandPrimary is called when exiting the literalPrmrAltCommandPrimary production.
	ExitLiteralPrmrAltCommandPrimary(c *LiteralPrmrAltCommandPrimaryContext)

	// ExitGstringPrmrAltCommandPrimary is called when exiting the gstringPrmrAltCommandPrimary production.
	ExitGstringPrmrAltCommandPrimary(c *GstringPrmrAltCommandPrimaryContext)
	...
	// ExitNamedPropertyArgList is called when exiting the namedPropertyArgList production.
	ExitNamedPropertyArgList(c *NamedPropertyArgListContext)

	// ExitMapEntry is called when exiting the mapEntry production.
	ExitMapEntry(c *MapEntryContext)

	// ExitNamedPropertyArg is called when exiting the namedPropertyArg production.
	ExitNamedPropertyArg(c *NamedPropertyArgContext)
	...
	// ExitNamedPropertyArgLabel is called when exiting the namedPropertyArgLabel production.
	ExitNamedPropertyArgLabel(c *NamedPropertyArgLabelContext)

	// ExitNamedArgLabel is called when exiting the namedArgLabel production.
	ExitNamedArgLabel(c *NamedArgLabelContext)
	...
	// ExitEnhancedArgumentListInPar is called when exiting the enhancedArgumentListInPar production.
	ExitEnhancedArgumentListInPar(c *EnhancedArgumentListInParContext)

	// ExitFirstArgumentListElement is called when exiting the firstArgumentListElement production.
	ExitFirstArgumentListElement(c *FirstArgumentListElementContext)
