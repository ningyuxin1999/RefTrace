...
func (v *ASTBuilder) VisitParExpression(ctx *ParExpressionContext) interface{} {
	...
	expression.PutNodeMetaData(INSIDE_PARENTHESES_LEVEL, level.(int)+1)
...
func (v *ASTBuilder) VisitLambdaExpression(ctx *LambdaExpressionContext) interface{} {
	v.pushSwitchExpressionRuleContext(ctx)
	defer v.popSwitchExpressionRuleContext()
	params := v.VisitLambdaParameters(ctx.LambdaParameters().(*LambdaParametersContext)).([]*Parameter)
	body := v.VisitLambdaBody(ctx.LambdaBody().(*LambdaBodyContext)).(Statement)
	lambda := NewLambdaExpression(params, body)

	return configureAST(lambda, ctx)
}
...
func (v *ASTBuilder) VisitClosureOrLambdaExpression(ctx *ClosureOrLambdaExpressionContext) interface{} {
	if ctx.Closure() != nil {
		...
		lambdaCtx := ctx.LambdaExpression()
		lambda := v.VisitLambdaExpression(lambdaCtx.(*LambdaExpressionContext)).(*LambdaExpression)
		return configureAST(lambda, ctx)
	}
